...
setwd("C:/Users/Moehler-M/Documents/R")
data_folder <- "geo_guidelines_case_study"

source("geo_guidelines_case_study/functions_ckm_raster.R")
source("geo_guidelines_case_study/functions_infoloss_raster.R")
...
ggplot(r_locIfa, aes(x, y, fill = locI)) +
  geom_tile() +
  facet_grid(variant ~ fa) +
  scale_fill_viridis_c(direction = 1, option = "mako", name = "loc. I", trans = "log10") +
  guides(fill = guide_colorbar(barheight = 15, barwidth = 0.5)) +
  scale_x_continuous(name = NULL, expand = c(0, 0)) +
  scale_y_continuous(name = NULL, expand = c(0, 0)) +
  theme_bw() +
